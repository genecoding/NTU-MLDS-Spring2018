...
class Solver(object):
    ...
    def _train_epoch(self, data_iter):
        ...
        CLIP = 1
        epoch_loss = 0
        for i, (src, trg) in enumerate(data_iter):
    ...
    def train(self, train_iter, valid_iter, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            if valid_loss < best_valid_loss:
                ...
                self._save_model('seq2seq_model_bestscore.pt')

            print(f'Epoch: {epoch+1:02} | Time: {epoch_mins}m {epoch_secs}s')
            print(f'\tTrain Loss: {train_loss:.3f} | Train PPL: {math.exp(train_loss):7.3f}')
            print(f'\t Val. Loss: {valid_loss:.3f} |  Val. PPL: {math.exp(valid_loss):7.3f}')

        self._save_model('seq2seq_model_final.pt')
        
    def evaluate(self, data_iter):
        loss = self._evaluate_epoch(data_iter)
        print(f'| Test Loss: {loss:.3f} | Test PPL: {math.exp(loss):7.3f} |')

    def _save_model(self, save_name, save_path='/content/saved_model'):
