...
class Agent_DQN(Agent):
    def __init__(self, env, args):
        ...
        self.model = DQN(INPUT_SHAPE, HID_DIM, OUTPUT_DIM).to(self.device)
        ...
        self.env = env
        self.args = args
        self.logger = SummaryWriter('./log/4-2/dqn')
        self.optimizer = optim.Adam(self.model.parameters(), lr=args.lr)
        self.replay_buffer = ReplayBuffer(10000)
        self.target_model = DQN(INPUT_SHAPE, HID_DIM, OUTPUT_DIM).to(self.device)
    ...
    def train(self):
        ...
        while step < num_steps+1 and not early_stop:
            ...
            if done:
                ...
                self.logger.add_scalars('DQN', log, episode)
    ...
    def _save_model(self, save_path='./saved_models/dqn'):
        os.makedirs(save_path, exist_ok=True)
        torch.save(self.model.state_dict(), os.path.join(save_path, 'model_dqn.pt'))
        # print(f'Save model to {save_path}')


    def _load_model(self, load_path='./saved_models/dqn'):
        self.model.load_state_dict(torch.load(os.path.join(load_path, 'model_dqn.pt'), map_location=self.device))
    ...
    def _update_model(self, batch_size, gamma=0.99):
        ...
        next_q_values = next_q_preds.max(dim=1).values.detach()
