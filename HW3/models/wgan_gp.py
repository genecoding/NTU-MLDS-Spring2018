...
class WGAN_GP(nn.Module):
    def __init__(self, channels=3, noise_dim=100, n_critic=5, lambda_=10):
        super().__init__()
        
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.logger = SummaryWriter('./log/wgan-gp')
    ...
    def _gradient_penalty(self, real_images, fake_images):
        ...
        logtis_inter = self.D(interpolated_imgs)
    ...
    def train(self, data_loader, save_imgname='wgan-gp_result.jpg', show_every=250, batch_size=64, num_epochs=10):
        iter_count = 0
        for epoch in range(num_epochs):
            for i, (x, _) in enumerate(data_loader):
                ...
                logits_real = self.D(real_images)
                ...
                d_total_error = self._discriminator_loss(logits_real, logits_fake) \
                                + self.lambda_ * self._gradient_penalty(real_images, fake_images)
    ...
    def test(self, batch_size=25, save_imgname='wgan-gp_gen.jpg'):
    ...
    def save_model(self, save_path='./saved_model/wgan-gp'):
    ...
    def load_model(self, load_path='./saved_model/wgan-gp'):
